...
router.get('/:id', verificarToken, soloProfesor, async (req: AuthRequest, res: Response): Promise<void> => {
  const plan = await db.query(
    `SELECT p.*, p.hora_inicio, p.duracion_minutos, s.nombre AS sala, s.id AS sala_id, m.nombre AS materia, m.id AS materia_id
     FROM planificaciones p
     JOIN salas s ON s.id = p.sala_id
     JOIN materias m ON m.id = p.materia_id
...
router.post('/', verificarToken, soloProfesor, async (req: AuthRequest, res: Response): Promise<void> => {
  const { titulo, salaId, materiaId, fecha, horaInicio, duracionMinutos, temaIds } = req.body
  ...
  const resultado = await db.query(
    `INSERT INTO planificaciones (titulo, profesora_id, sala_id, materia_id, fecha, hora_inicio, duracion_minutos)
     VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
    [titulo, req.usuario!.id, salaId, materiaId, fecha || null, horaInicio || null, duracionMinutos || 45]
  )
...
router.put('/:id', verificarToken, soloProfesor, async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  await db.query(
    `UPDATE planificaciones SET titulo = COALESCE($1, titulo), sala_id = COALESCE($2, sala_id),
     materia_id = COALESCE($3, materia_id), fecha = $4, hora_inicio = $5, duracion_minutos = COALESCE($6, duracion_minutos)
     WHERE id = $7 AND profesora_id = $8`,
    [titulo, salaId, materiaId, fecha || null, horaInicio || null, duracionMinutos, req.params.id, req.usuario!.id]
  )
